...
namespace HtmlHelp.ChmDecoding
{
	...
	public sealed class CHMFile : IDisposable
	{
		...
		/// <summary>
		/// Reads a file from the CHM storage by its local path.
		/// </summary>
		/// <param name="localPath">Local path inside the CHM (e.g. "index.htm", "images/logo.png").</param>
		/// <returns>Returns the file bytes, or null if the file cannot be found/opened.</returns>
		public byte[] ReadFileBytes(string localPath)
		{
			if(string.IsNullOrEmpty(localPath))
				return null;

			// Normalize: CHM locals usually use '/', while callers may pass leading '/' or '\\'.
			...
			try
			{
				fileObject = ChmWrapper.OpenUCOMStream(null, localPath);
