...
namespace HtmlHelp.WinForms.Controls
{
	...
	public class helpIndex : System.Windows.Forms.UserControl
	{
		...
		public void BuildIndex(HtmlHelp.Index index, HtmlHelp.IndexType typeOfIndex)
		{
		...
		public void BuildIndex(HtmlHelp.Index index, HtmlHelp.IndexType typeOfIndex, InfoTypeCategoryFilter filter)
		{
			switch(typeOfIndex)
			{
				case HtmlHelp.IndexType.AssiciativeLinks: _arrIndex = index.ALinks; break;
				case HtmlHelp.IndexType.KeywordLinks: _arrIndex = index.KLinks; break;
			}

			lbIndex.Items.Clear();

			foreach(HtmlHelp.IndexItem curItem in _arrIndex)
			{
				bool bAdd = true;
		...
		private void DisplayTopic(bool errorOnNothingSelected)
		{
			...
			{
					HtmlHelp.IndexItem item = (HtmlHelp.IndexItem) _arrIndex[lbIndex.SelectedIndex];
				...
				else 
				{
					if(item.IsSeeAlso)
					{
						if( item.SeeAlso.Length>0)
						{
							...
							if (item.Topics.Count == 1)
							{
								url = ((HtmlHelp.IndexTopic)item.Topics[0]).URL;
								title = ((HtmlHelp.IndexTopic)item.Topics[0]).Title;
							}
					...
					if (item.Topics.Count == 1)
					{
						OnIndexSelected( new IndexEventArgs( ((HtmlHelp.IndexTopic)item.Topics[0]).Title, ((HtmlHelp.IndexTopic)item.Topics[0]).URL, item.IsSeeAlso, item.SeeAlso) );
					}
				}
			} 
			else 
