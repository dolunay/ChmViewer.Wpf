        ...
        public static void CefInitialize()
        {
            ...
            if (Cef.IsInitialized != true)
            {
                ...
                if (!initialized)
                {
                    ...
                    {
                        //Log.Error($"Cef.Initialize failed with {exitCode}, another process is already using cache path {cachePath}");
                        MessageBox.Show($"Cef.Initialize failed with {exitCode}, another process is already using cache path {cachePath}");
                    }
                    else
                    {
                        //Log.Error($"Cef.Initialize failed with {exitCode}, check the log file for more details.");
